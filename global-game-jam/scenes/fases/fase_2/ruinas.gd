...
@export var mascara_camaleao: Masks 
@export var mascara_morcego: Masks
...
func _process(_delta: float) -> void:
	if Dialogic.VAR.get_variable("pegouMascaraCamaleao"):
		GameManager.player_ref.setCmalaeonMask(mascara_camaleao)
	if Dialogic.VAR.get_variable("pegouMascaraMorcego"):
		GameManager.player_ref.setBatMask(mascara_morcego)
